...
class OverlayFormattingTest {
    ...
    fun `formatSegmentMetric with null returns placeholder`() {
        val result = SegmentFormatters.formatSegmentMetric(null)
        assertEquals("\u2014", result)
    }

    @Test
    fun `formatSegmentMetric returns two lines separated by newline`() {
        val result = SegmentFormatters.formatSegmentMetric(makeMetric())
    ...
    fun `formatSegmentMetric first line contains DL duration`() {
        val result = SegmentFormatters.formatSegmentMetric(makeMetric(totalDurationMs = 200L))
    ...
    fun `formatSegmentDetails contains size throughput and optional ttfb`() {
        ...
        val result =
            SegmentFormatters.formatSegmentDetails(
    ...
    fun `formatSegmentDetails omits TTFB when networkTiming is null`() {
        val result = SegmentFormatters.formatSegmentDetails(makeMetric(networkTiming = null))
        assertTrue(!result.contains("TTFB"), "Expected no 'TTFB' in: $result")
    }

    @Test
    fun `formatSegmentMetric second line contains throughput`() {
        val result = SegmentFormatters.formatSegmentMetric(makeMetric(throughputBytesPerSec = 3_800_000L))
    ...
    fun `formatSegmentMetric includes TTFB on second line when networkTiming present`() {
        val timing = NetworkTiming(ttfbMs = 40L, transferDurationMs = 160L, isEstimated = true)
        val result = SegmentFormatters.formatSegmentMetric(makeMetric(networkTiming = timing))
        val line2 = result.lines()[1]
    ...
    fun `formatSegmentMetric omits TTFB when networkTiming is null`() {
        val result = SegmentFormatters.formatSegmentMetric(makeMetric(networkTiming = null))
        assertTrue(!result.contains("TTFB"), "Expected no 'TTFB' in: $result")
    }

    @Test
    fun `formatSegmentMetric formats large size in MB`() {
        val result = SegmentFormatters.formatSegmentMetric(makeMetric(sizeBytes = 1_200_000L))
        assertTrue(result.contains("1.2 MB"), "Expected '1.2 MB' in: $result")
    }
    ...
    fun `formatTtfb with null returns dash`() {
        assertEquals("—", TimeFormatters.formatTtfb(null))
    }

    @Test
    fun `formatTtfb with estimated timing prefixes tilde`() {
        val result = TimeFormatters.formatTtfb(NetworkTiming(ttfbMs = 40L, transferDurationMs = 160L, isEstimated = true))
        assertEquals("~40ms", result)
    }

    @Test
    fun `formatTtfb with measured timing has no tilde prefix`() {
        val result = TimeFormatters.formatTtfb(NetworkTiming(ttfbMs = 40L, transferDurationMs = 160L, isEstimated = false))
        assertEquals("40ms", result)
    }
    ...
    fun `formatAbrSwitch with resolution change shows height labels`() {
        ...
        val result = TrackFormatters.formatAbrSwitch(from, to)
    ...
    fun `formatAbrSwitch with null from shows dash arrow label`() {
        ...
        val result = TrackFormatters.formatAbrSwitch(null, to)
        assertTrue(result.contains("720p"), "Expected '→ 720p' in: $result")
    }

    @Test
    fun `formatBufferDuration formats seconds`() {
        val result = TimeFormatters.formatBufferDuration(12_400L)
    ...
    fun `formatRelativeTimestamp formats minutes and seconds`() {
        val base = 1_000L
        val result = TimeFormatters.formatRelativeTimestamp(base + 62_000L, base)
    ...
    fun `formatRelativeTimestamp zero offset returns plus zero`() {
        val base = 5_000L
        val result = TimeFormatters.formatRelativeTimestamp(base, base)
        assertEquals("+0:00", result)
    }

    @Test
    fun `formatSwitchReason returns correct label for each enum value`() {
        assertEquals("INITIAL", TrackFormatters.formatSwitchReason(SwitchReason.INITIAL))
        assertEquals("ADAPTIVE", TrackFormatters.formatSwitchReason(SwitchReason.ADAPTIVE))
        assertEquals("MANUAL", TrackFormatters.formatSwitchReason(SwitchReason.MANUAL))
        assertEquals("TRICKPLAY", TrackFormatters.formatSwitchReason(SwitchReason.TRICKPLAY))
        assertEquals("UNKNOWN", TrackFormatters.formatSwitchReason(SwitchReason.UNKNOWN))
    }
    ...
    fun `formatErrorCategory returns short label for each category`() {
        assertEquals("LOAD", ErrorFormatters.formatErrorCategory(ErrorCategory.LOAD_ERROR))
        assertEquals("CODEC", ErrorFormatters.formatErrorCategory(ErrorCategory.VIDEO_CODEC_ERROR))
        assertEquals("FRAMES", ErrorFormatters.formatErrorCategory(ErrorCategory.DROPPED_FRAMES))
        assertEquals("AUDIO", ErrorFormatters.formatErrorCategory(ErrorCategory.AUDIO_SINK_ERROR))
        assertEquals("ACODEC", ErrorFormatters.formatErrorCategory(ErrorCategory.AUDIO_CODEC_ERROR))
    }
    ...
    fun `formatAbsoluteTimestamp returns HH mm ss SSS format`() {
        ...
        val result = TimeFormatters.formatAbsoluteTimestamp(0L)
    ...
    fun `formatErrorsForExport produces correct header and rows`() {
        ...
        val result = ErrorFormatters.formatErrorsForExport(errors, base)
    ...
    fun `formatErrorsForExport with empty list returns just header`() {
        val result = ErrorFormatters.formatErrorsForExport(emptyList(), 0L)
    ...
    fun `formatActiveAudio with null returns Loading`() {
        assertEquals("Loading\u2026", TrackFormatters.formatActiveAudio(null))
    }
    ...
    fun `formatActiveAudio with only language contains display language`() {
        ...
        val result = TrackFormatters.formatActiveAudio(makeAudio(language = "en"))
    ...
    fun `formatActiveAudio with no language and no label returns Unknown`() {
        val result =
            TrackFormatters.formatActiveAudio(
    ...
    fun `formatActiveSubtitle with null returns Off`() {
        assertEquals("Off", TrackFormatters.formatActiveSubtitle(null))
    }
    ...
    fun `formatActiveSubtitle with language shows display language`() {
        ...
        val result = TrackFormatters.formatActiveSubtitle(makeSubtitle(language = "tr"))
    ...
    fun `formatActiveSubtitle CC adds the CC suffix`() {
        val result = TrackFormatters.formatActiveSubtitle(makeSubtitle(kind = SubtitleKind.CC))
        assertTrue(result.contains("(CC)"), "Expected '(CC)' in: $result")
    }

    @Test
    fun `formatActiveSubtitle SIDECAR does not add the CC suffix`() {
        val result = TrackFormatters.formatActiveSubtitle(makeSubtitle(kind = SubtitleKind.SIDECAR))
        assertTrue(!result.contains("(CC)"), "Expected no '(CC)' in: $result")
    }

    @Test
    fun `formatActiveSubtitle shows WebVTT for vtt mime type`() {
        val result = TrackFormatters.formatActiveSubtitle(makeSubtitle(mimeType = "text/vtt"))
        assertTrue(result.contains("WebVTT"), "Expected 'WebVTT' in: $result")
    }

    @Test
    fun `formatActiveSubtitle shows TTML for ttml mime type`() {
        val result = TrackFormatters.formatActiveSubtitle(makeSubtitle(mimeType = "application/ttml+xml"))
    ...
    fun `formatActiveSubtitle with no language and no label returns Unknown`() {
        val result =
            TrackFormatters.formatActiveSubtitle(
    ...
    fun `segmentTrackBadge maps each track type`() {
        assertEquals("V", SegmentFormatters.segmentTrackBadge(SegmentTrackType.VIDEO))
        assertEquals("A", SegmentFormatters.segmentTrackBadge(SegmentTrackType.AUDIO))
        assertEquals("T", SegmentFormatters.segmentTrackBadge(SegmentTrackType.TEXT))
        assertNull(SegmentFormatters.segmentTrackBadge(SegmentTrackType.UNKNOWN))
    }

    @Test
    fun `formatSegmentMetric first line includes track label and extension`() {
        val result =
            SegmentFormatters.formatSegmentMetric(
    ...
    fun `segmentExtension reads a plain uri`() {
        assertEquals("ts", SegmentFormatters.segmentExtension("https://host/path/seg3.ts"))
    }

    @Test
    fun `segmentExtension strips query string`() {
        assertEquals("ts", SegmentFormatters.segmentExtension("https://host/path/seg3.ts?token=abc"))
    }

    @Test
    fun `segmentExtension strips fragment`() {
        assertEquals("m4s", SegmentFormatters.segmentExtension("https://host/path/seg3.m4s#frag"))
    }

    @Test
    fun `segmentExtension returns null for extensionless path`() {
        assertNull(SegmentFormatters.segmentExtension("https://host/path/segment"))
    }

    @Test
    fun `segmentExtension returns null for unknown uri`() {
        assertNull(SegmentFormatters.segmentExtension("(unknown)"))
    }

    @Test
    fun `segmentExtension returns null for overlong extension`() {
        assertNull(SegmentFormatters.segmentExtension("https://host/path/file.megabyte"))
    }
}
